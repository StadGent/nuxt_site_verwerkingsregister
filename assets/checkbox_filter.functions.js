...
(function (root, factory) {
  ...
  return function (elem, options) {
    ...
    const filter = (clear) => {
      ...
      [].slice.call(checkboxContainers).forEach((container) => {
        const displayedCount = container.querySelectorAll(`${options.checkboxes || 'div.checkbox'}:not([hidden])`).length
        if (displayedCount) {
    ...
    const removeTag = (checkbox) => {
      const test = selectedContainer.querySelectorAll('.filter')
    ...
    const checkboxLoop = (next) => {
      for (let i = checkboxes.length; i--;) {
        const checkboxWrapper = checkboxes[i]

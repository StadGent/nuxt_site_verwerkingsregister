<template>
  <article class="detail-layout">
    ...
    <h2>Gegevens</h2>
    <h3 v-if="item.personalDataDescription || item.sensitivePersonalDataDescription">
      Over wie?
    </h3>
    <p v-if="item.personalDataDescription">
      {{ item.personalDataDescription.value }}
    </p>
    <p v-if="item.sensitivePersonalDataDescription">
      {{ item.sensitivePersonalDataDescription.value }}
    </p>
    <h3>Welke persoonsgegevens?</h3>
    <ul v-if="item.personalData && item.personalData.value.length > 0">
      <li v-for="(value, index) in item.personalData.value" :key="index">
        {{ value }}
      </li>
    </ul>
    <p v-else>
      Er zijn geen persoonsgegevens van toepassing.
    </p>
    <h3>Welke gevoelige persoonsgegevens?</h3>
    <ul v-if="item.sensitivePersonalData && item.sensitivePersonalData.value.length > 0">
      <li v-for="(value, index) in item.sensitivePersonalData.value" :key="index">
        {{ value }}
      </li>
    </ul>
    <p v-else>
    ...
    <h3 v-if="item.formal_framework_clarification">
      Verduidelijking
    </h3>
    <p v-if="item.formal_framework_clarification">
      {{ item.formal_framework_clarification.value }}
    </p>
    <h2 v-if="item.grantees && item.grantees.value.length">
      Ontvangers
    </h2>
    <ul v-if="item.grantees && item.grantees.value.length" :class="item.grantees.value.length > 5 ? 'columns' : null">
      <li v-for="(value, index) in item.grantees.value" :key="index">
        {{ value }}
      </li>
    </ul>
    <h2>Bewaartermijn</h2>

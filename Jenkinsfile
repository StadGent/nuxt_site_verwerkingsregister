...
podTemplate(cloud: 'openshift') {
    ...
    node('maven') {
        stage('Deploy to DV') {
            ...
            openshiftTag alias: 'false', destStream: 'verwerkingsregister', destTag: 'accepted_dv', destinationNamespace: 'verwerkingsregisterdv', namespace: 'servicefactorydv', srcStream: 'verwerkingsregister', srcTag: 'latest'
        ...
        try {
        	stage ('Import image from QA') {
	            withCredentials([string(credentialsId: 'verwerkingsregisterpr-dg-image-manager-secret', variable: 'token')]) {
	                ...
	                sh 'oc import-image verwerkingsregister:latest --from registry.servicesqa.local/verwerkingsregisterdv/verwerkingsregister --confirm --insecure=true'
	            }
        	}
